...
7. UI PATTERN — exactly one of: "visual chips" (types that look different) · "multi-select checkboxes" (materials, features, most attributes) · "range buckets" (numeric presets) · "range slider with presets" (price) · "single-select" (few exclusive options) · "location search" · "toggle" (yes/no) · "display only" (every Tier 3 item).
